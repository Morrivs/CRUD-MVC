<?php
	...
	class userController extends mainModel{
		...
		public function registrarUsuarioControlador(){
            ...
            if($_FILES['usuario_foto']['name']!="" && $_FILES['usuario_foto']['name']>0){
            ...
            //si se registro un usuario si se guardo
        ...
        public function eliminarUsuarioControlador(){
            ...
            if($id==1){
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No podemos el usuario principal del sistema",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            //verificar usuario
            $datos->ejecutarConsulta("SELECT * FROM usuario WHERE usuario_id = '$id'");
            ...
            if($eliminarUsuario->rowCount()==1){
                ...
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No se pudo eliminar el usuario".$datos['usuario_nombre']." ".$$datos['usuario_apellido']." de el sistema, por favor intente nuevamente",
                ];
            }
            return json_encode($alerta);
            ...
            if($check_admin->rowCount()==1){
				...
				if($check_admin['usuario_usuario']!=$admin_usuario || !password_verify($admin_clave,$check_admin['usuario_clave'])){
					$alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"USUARIO o CLAVE de ADMINISTRADOR incorrecto",
						"icono"=>"error"
					];
					return json_encode($alerta);
					exit();
		...
		public function actualizarFotoUsuarioControlador(){
			...
			if($this->actualizarDatos("usuario", $usuario_datos_up, $condicion)){
                ...
                $alerta=[
                    "tipo"=>"recargar",
                    "titulo"=>"Foto Actualizada!",
                    "texto"=>"No hemos podido actualizar los datos del Usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." sin embargo la foto se actualizo con exito",
                    "icono"=>"warning"
                ];
            }
            return json_encode($alerta);
		...
		public function eliminarFotoUsuarioControlador(){
			...
			if(is_file($img_dir.$datos['usuario_foto'])){
				...
				if(!unlink($img_dir.$datos['usuario_foto'])){
					$alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
						"texto"=>"Error al eliminar la foto del usuario, por favor intente nuevamente",
						"icono"=>"error"
					];
					return json_encode($alerta);
					exit();
			...
			if($this->actualizarDatos("usuario", $usuario_datos_up, $condicion)){
                ...
                $alerta=[
                    "tipo"=>"recargar",
                    "titulo"=>"Foto Actualizada!",
                    "texto"=>"No hemos podido actualizar los datos del Usuario ".$datos['usuario_nombre']." ".$datos['usuario_apellido']." sin embargo la foto se elimino con exito",
                    "icono"=>"warning"
                ];
            }
            return json_encode($alerta);
